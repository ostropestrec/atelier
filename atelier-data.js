...
/** Akordeon Kurzy vs. detail kurzu — stejné courseId, jiné DOM id (oba screeny jsou v DOM najednou). */
function _bookingPayId(kind, courseId, scope = 'card') {
  return scope === 'detail' ? `${kind}-detail-${courseId}` : `${kind}-${courseId}`
}

function _bookingPayEl(kind, courseId, scope = 'card') {
  return document.getElementById(_bookingPayId(kind, courseId, scope))
}

function _activeBookingPayScope(courseId) {
  const detail = document.getElementById('screen-detail-kurzu')
  if (detail?.classList.contains('active') && String(window._detailCourseId) === String(courseId)) {
    return 'detail'
  }
  return 'card'
}

function _eachBookingPayScope(courseId, fn) {
  for (const scope of ['detail', 'card']) fn(scope)
}
...
function _restrictedBadgeHtml() {
  return `<span class="badge" style="background:#E8EEF8;color:#2854B9;font-size:11px;font-weight:600;">${_escHtml(_tp('courses.badgeRestricted'))}</span>`
}

function _workshopBadgeHtml() {
  return `<span class="badge" style="background:#FFF4E0;color:#8B5C00;font-size:11px;font-weight:600;">${_escHtml(_tp('courses.badgeWorkshop'))}</span>`
}
...
export function renderKurzy() {
  ...
  if (!window.AppState.courses.length) {
    container.insertAdjacentHTML('beforeend', `
      <div style="padding:40px;text-align:center;font-size:13px;color:#9b9b9b;">
    ...
    card.innerHTML = `
      <div class="cm" onclick="toggleC('${c.id}')">
        ...
        <div class="cr">
          ...
          <span style="font-size:12px;color:#6b6b6b;" id="cv-${c.id}">›</span>
        </div>
      </div>
      ...
      <div class="cex" id="cx-${c.id}">
        <div class="cxi">
          <div class="cxi-left">
            ${buildCourseImage(c)}
            <div style="font-size:12px;color:#6b6b6b;line-height:1.6;margin-bottom:10px;">${desc}</div>
...
function buildCourseImage(c) {
  ...
  return url
    ...
    : `<div style="background:#F8F8F8;border-radius:8px;height:78px;display:flex;align-items:center;justify-content:center;margin-bottom:10px;font-size:11px;color:#9b9b9b;">
...
function buildTermPills(upcoming, color, courseId, interactive = true, large = false, forDetail = false) {
  if (!upcoming.length) {
    return `<span style="font-size:11px;color:#9b9b9b;">${_tp('courses.noDates')}</span>`
  }
...
function _buildCourseBookingInline(course, courseId, color, upcoming, bookable, opts = {}) {
  ...
  if (!hasSpots) {
    return `<div style="margin-top:8px;text-align:center;font-size:13px;color:#791F1F;background:#fdeaea;padding:10px;border-radius:10px;">
      ${_escHtml(_tp('courses.allSessionsFull'))}
    </div>`
  }

  const termsInteractive = !isWorkshopBundle
  const forDetail = !!wrapSection
  const pillsHtml = buildTermPills(upcoming, color, courseId, termsInteractive, true, forDetail)
  ...
  const btnStyle = btnFullWidth
    ? `background:${color};width:100%;padding:14px;border:none;font-size:16px;font-weight:600;cursor:pointer;margin-top:14px;`
    : `background:${color};margin-top:8px;`

  const inner = `
    <div class="blbl" style="margin-bottom:8px;">${_escHtml(sectionLabel)}</div>
    ${isWorkshopBundle ? `<p style="font-size:13px;color:var(--muted);margin:0 0 10px;line-height:1.5;">${_escHtml(_tp('courses.workshopSessionsNote', { n: upcoming.length }))}</p>` : ''}
...
function _buildKurzyAccordionBooking(course, color, upcoming, bookable) {
  ...
  return `
      <div class="blbl" style="margin-bottom:8px;">${_escHtml(sectionLabel)}</div>
      <div style="font-size:13px;color:#6b6b6b;line-height:1.5;margin-top:4px;padding:10px 12px;background:#f5f5f5;border-radius:10px;">${_escHtml(_tp('courses.restrictedBookingHint'))}</div>`
  }
  if (!upcoming.length) {
    return `
      <div class="blbl" style="margin-bottom:8px;">${_escHtml(sectionLabel)}</div>
      <span style="font-size:11px;color:#9b9b9b;">${_tp('courses.noDates')}</span>`
  }
  return _buildCourseBookingInline(course, course.id, color, upcoming, bookable, { sectionLabel })
}

function buildBuyPanel(c, color, includeReserveButton = true, payScope = 'card') {
  if (_isStaffUser()) return ''
  const singleId = _bookingPayId('buy-single', c.id, payScope)
  const panelId = _bookingPayId('pass-panel', c.id, payScope)
  const countId = _bookingPayId('card-pass-count', c.id, payScope)
  const msgId = _bookingPayId('card-msg', c.id, payScope)
  const resId = _bookingPayId('res-btn', c.id, payScope)
  return `
    ...
    <div class="bo bo-pay"
      ...
      <div class="brow">
        ...
        <span style="font-size:12px;font-weight:500;color:${color};">${fmtPrice(c.price_single)}</span>
      </div>
      <div class="bsb">${c.is_workshop ? '' : _tp('booking.payment.singleSessionValidity')}</div>
    </div>
    <div id="${panelId}" class="pay-opts-pass-list"></div>
    <div id="${countId}" style="display:none;font-size:12px;color:#6b6b6b;margin:8px 0 2px;line-height:1.45;text-align:center;"></div>
    <div id="${msgId}" style="display:none;border-radius:8px;padding:8px 12px;font-size:12px;text-align:center;margin-top:4px;"></div>
    ...
    return `
      <div class="bo bo-pay"
        ...
        <div class="brow">
          ...
          <span style="font-size:12px;font-weight:500;color:${pc};">${fmtPrice(p.price)}</span>
        </div>
...
async function loadPassesForCourse(courseId, payScope = null) {
  const scope = payScope ?? _activeBookingPayScope(courseId)
  const panel     = _bookingPayEl('pass-panel', courseId, scope)
  const singleDiv = _bookingPayEl('buy-single', courseId, scope)
  if (!panel) return
    ...
    panel.innerHTML = ownedPasses.map((up, i) => {
      ...
      return `
        <div class="bo bo-pay"
          ...
          <div class="brow">
            <span class="bnm">${name}</span>
            <span style="font-size:12px;font-weight:600;color:${pc};">${up.entries_remaining} ${entriesWord(_locale(), up.entries_remaining)}</span>
          </div>
...
async function _loadPassesForExpandedKurzyCard(courseId) {
  ...
  try {
    await loadPassesForCourse(courseId, 'card')
  } catch (e) {
      ...
      box.innerHTML = bundleLessons.map(l => {
        ...
        if (enrolled || full) {
          ...
          return `<div class="bk-lesson-enrolled" style="display:flex;align-items:flex-start;gap:10px;padding:8px 0;border-bottom:0.5px solid rgba(0,0,0,.06);opacity:.55;color:#9b9b9b;">
            <span style="font-size:13px;flex:1;">${_escHtml(line)}</span>
            <span style="font-size:11px;font-weight:700;padding:2px 8px;border-radius:20px;background:#E8EEF9;color:#2854B9;white-space:nowrap;">${_escHtml(tag)}</span>
          </div>`
        }
        return `<label style="display:flex;align-items:flex-start;gap:10px;padding:8px 0;border-bottom:0.5px solid rgba(0,0,0,.06);opacity:.85;">
          ...
          <span style="font-size:13px;">${_escHtml(line)}</span>
        </label>`
      }).join('')
    }
    if (hint) {
  ...
  if (box) {
    box.innerHTML = courseLessons.length
      ? courseLessons.map(l => {
          ...
          if (enrolled) {
            return `<div class="bk-lesson-enrolled" style="display:flex;align-items:flex-start;gap:10px;padding:8px 0;border-bottom:0.5px solid rgba(0,0,0,.06);opacity:.55;color:#9b9b9b;">
              <span style="font-size:13px;flex:1;">${_fmtBkLessonLine(l)}</span>
              <span style="font-size:11px;font-weight:700;padding:2px 8px;border-radius:20px;background:#E8EEF9;color:#2854B9;white-space:nowrap;">${_escHtml(_tp('common.enrolled'))}</span>
            </div>`
          }
          ...
          return `<label style="display:flex;align-items:flex-start;gap:10px;padding:8px 0;border-bottom:0.5px solid rgba(0,0,0,.06);cursor:pointer;">
            ...
            <span style="font-size:13px;">${_fmtBkLessonLine(l)} <span style="color:#6b6b6b;">(${_tp('booking.option.spotsSuffix', { n: l.available_spots })})</span></span>
          </label>`
        }).join('')
      : `<div style="font-size:13px;color:#9b9b9b;">${_escHtml(_tp('booking.empty.noScheduledSessions'))}</div>`
  }
...
async function renderCourseDetail(courseId) {
  ...
  el.innerHTML = `
    <div style="max-width:760px;">
      ...
      ${restricted && !bookable && !_isStaffUser()
        ? `<div style="font-size:14px;color:#2854B9;background:#E8EEF8;padding:12px 14px;border-radius:10px;margin-bottom:16px;line-height:1.55;">${_escHtml(_tp('courses.restrictedBookingHint'))}</div>`
        : ''}

      ${isWorkshopBundle && !(!_isStaffUser() && bookable && upcoming.length)
        ? `<p style="font-size:14px;color:var(--muted);margin:0 0 12px;line-height:1.5;">${_escHtml(_tp('courses.workshopSessionsNote', { n: upcoming.length }))}</p>`
        : ''}
  ...
  if (!_isStaffUser() && bookable && upcoming.length && upcoming.some(l => l.available_spots > 0)) {
    ...
    if (currentUser && !bundle) {
      try {
        await loadPassesForCourse(courseId, 'detail')
      } catch (e) {
...
window.pickTerm = (el, color, courseId) => {
  ...
  if (st.paymentType === 'pass' && st.passId) {
    ...
    const n = arr.length
    const bookLabel = n ? _tp('booking.btn.bookSelected', { n }) : _tp('booking.btn.book')
    _eachBookingPayScope(courseId, scope => {
      const btn = _bookingPayEl('res-btn', courseId, scope)
      if (btn) btn.textContent = bookLabel
    })
    _refreshCardPassSlotsRow(courseId)
    _refreshCardPassSlotsRow(courseId)
    return
...
window.reserveFromCard = async (courseId) => {
  ...
  const btn = _bookingPayEl('res-btn', courseId, _activeBookingPayScope(courseId))
    ?? _bookingPayEl('res-btn', courseId, 'card')
...
export async function buildStaffLessonsSectionHtml({
          ...
          <div class="staff-term-card" style="border:1px solid ${color};border-radius:12px;overflow:hidden;margin-bottom:8px;background:#fff;${isOff ? 'opacity:.75;' : ''}">
              ...
              <div style="flex:1;padding:12px 14px;display:flex;align-items:flex-start;gap:12px;">
                ...
                <div style="flex:1;min-width:0;display:flex;flex-direction:column;gap:6px;">
                  <div style="font-size:14px;font-weight:600;line-height:1.35;">
                    ${title}${course?.is_workshop ? ' <span style="font-size:10px;font-weight:700;padding:1px 6px;border-radius:20px;background:#FFF4E0;color:#8B5C00;">WORKSHOP</span>' : ''}
                    ${isOff ? '<span style="font-size:10px;font-weight:700;padding:1px 6px;border-radius:20px;background:#F3F4F6;color:#6b6b6b;margin-left:6px;">DEAKTIVOVÁNO</span>' : ''}
                  </div>
                  <div style="font-size:12px;color:#6b6b6b;">${dateStr} · ${timeStr}</div>
                  <div style="margin-top:2px;">
                    <div style="font-size:14px;font-weight:600;">${booked}/${cap}</div>
                    <div style="font-size:11px;color:#9b9b9b;margin-bottom:4px;">${_tp('courses.occupied')}</div>
            ...
            <div class="ml-cx" id="ml-cx-${lid}">
              <div style="padding:0 14px 14px 14px;">
                <div style="font-size:13px;color:#6b6b6b;line-height:1.65;overflow:hidden;margin-bottom:10px;">
      ...
      <div class="staff-term-card" style="border:1px solid ${color};border-radius:12px;background:#fff;margin-bottom:8px;padding:12px 14px;display:flex;align-items:center;justify-content:space-between;gap:12px;opacity:.75;">
        <div style="min-width:0;display:flex;align-items:flex-start;gap:12px;">
          ...
          <div style="min-width:0;">
            <div style="font-size:14px;font-weight:600;line-height:1.35;">${_escHtml(title)}</div>
            <div style="font-size:12px;color:#6b6b6b;margin-top:4px;">${_escHtml(dateStr)} · ${_escHtml(timeStr)}</div>
          </div>
        </div>
        <button type="button" class="btn-small" style="font-size:12px;padding:6px 10px;flex-shrink:0;"
  ...
  const renderPastPagination = total => {
    ...
    for (const n of pages) {
      if (last && n - last > 1) {
        btns.push(`<span style="font-size:13px;color:#9b9b9b;padding:0 2px;">…</span>`)
      }
      const activePage = n === current
      btns.push(`
        <button type="button"
          style="min-width:30px;height:30px;border-radius:999px;border:1px solid ${activePage ? 'var(--primary)' : 'var(--section-heading-accent)'};background:${activePage ? 'var(--primary)' : '#fff'};color:${activePage ? '#fff' : 'var(--section-heading-accent)'};font-size:12px;font-weight:700;cursor:pointer;"
  ...
  const sections = []
  if (active.length) {
    sections.push(`<div style="font-size:13px;color:#6b6b6b;margin-bottom:12px;">${active.length} aktivních termínů</div>`)
  ...
  if (deactivated.length) {
    const deactivatedBodyHtml = `
      <div style="font-size:13px;color:#6b6b6b;margin-bottom:12px;">${_escHtml(_tp('admin.lessonActions.futureDeactivatedHint', { n: deactivated.length }))}</div>
    ...
    container.innerHTML = `<div class="pass-shop-grid">${rows.map(p => {
      ...
      return `
        <div class="pass-shop-card" style="${surf}">
          ...
          <button type="button" class="btn-res" style="width:100%;padding:11px;border:none;font-size:14px;font-weight:600;cursor:pointer;background:${pc};"
